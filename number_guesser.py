...
num_gen = random.randint(1,100) #outputting a random integer in the range 1 and 100 inclusive
num_guess = int(input("Guess a number between 1 and 100: ")) #making the user guess an integer between 1 and 100
guesses = 1 #creating a variable to keep track on how many guesses the user has used

while num_gen != num_guess: #creating a while loop to allow the user to keep on guessing while their guess is not equal to the generated number
    if num_gen < num_guess: #giving relevant advice if the user guesses too high
        num_guess = int(input("Your guess was too high, try guessing a lower number: ")) #updating the variable numguess for the next round of checking
        guesses = guesses + 1 #updating the number of guesses
    
    elif num_gen > num_guess: #giving relevant avice if the user guesses too low
        num_guess = int(input("Your guess was too low, try guessing a higher number: ")) #updating the variable numguess for the next round of checking
        guesses = guesses + 1 #updating the number of guesses
